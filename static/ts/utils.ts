export function arcadeLocalMessage(message: string, type: string) {
...
export function arcadeServerMessage(message: string, type: string, data: any) {
    console.log(`${type}: ${message}`);
    ...
    if (userIdFromMessage === getCurrentUserId(data) && errorMessage) {
        arcadeLocalMessage(errorMessage, type);
    }
}
